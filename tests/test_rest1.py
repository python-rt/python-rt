...
class RtTestCase(unittest.TestCase):
    ...
    def test_login_and_logout(self):
        for name in self.RT_VALID_CREDENTIALS:
            tracker = rt.rest1.Rt(
                self.RT_VALID_CREDENTIALS[name]["url"],
            ...
            self.assertTrue(
                tracker.logout(), "Invalid logout from RT demo site " + name
            )
        for name, params in self.RT_INVALID_CREDENTIALS.items():
            tracker = rt.rest1.Rt(**params)
            self.assertFalse(
                tracker.login(),
                "Login to RT demo site " + name + " should fail but did not",
            )
            self.assertRaises(
                rt.exceptions.AuthorizationError, lambda: tracker.search()
            )
        ...
        for name, params in self.RT_BAD_URL.items():
            tracker = rt.rest1.Rt(**params)
            self.assertRaises(
                rt.exceptions.UnexpectedResponseError, lambda: tracker.login()
            )

    @unittest.skipUnless(
        _have_creds(RT_VALID_CREDENTIALS), "missing credentials required to run test"
    )
    def test_ticket_operations(self):
        ticket_subject = "Testing issue " + "".join(
        ...
        for name in ("RT4.4 stable",):
            ...
            default_password = self.RT_VALID_CREDENTIALS[name]["support"][
                "default_password"
            ]
            tracker = rt.rest1.Rt(
                url, default_login=default_login, default_password=default_password
            )
            self.assertTrue(tracker.login(), "Invalid login to RT demo site " + name)
            # unsecure cookie
            self._fix_unsecure_cookie(tracker)
            ...
            self.assertEqual(
                search_result,
                [],
                "Search for ticket with random subject returned non empty list.",
            )
            # create
            ...
            search_result = tracker.search(Subject=ticket_subject)
            self.assertEqual(
                len(search_result), 1, "Created ticket is not found by the subject."
            )
            self.assertEqual(
                search_result[0]["id"],
                "ticket/" + str(ticket_id),
                "Bad id in search result of just created ticket.",
            )
            self.assertEqual(
                search_result[0]["Status"],
                "new",
                "Bad status in search result of just created ticket.",
            )
            # search all queues
            search_result = tracker.search(
                Queue=rt.rest1.ALL_QUEUES, Subject=ticket_subject
            )
            self.assertEqual(
                search_result[0]["id"],
                "ticket/" + str(ticket_id),
            ...
            search_result = tracker.search(
                Queue=rt.rest1.ALL_QUEUES, raw_query=f'Subject="{ticket_subject}"'
            )
            self.assertEqual(
                search_result[0]["id"],
                "ticket/" + str(ticket_id),
            ...
            self.assertEqual(
                ticket,
                search_result[0],
                "Ticket get directly by its id is not equal to previous search result.",
            )
            # edit ticket
            ...
            ticket = tracker.get_ticket(ticket_id)
            self.assertEqual(
                ticket["Status"], "open", "Ticket status was not changed to open."
            )
            self.assertEqual(
                ticket["Requestors"],
                requestors,
                "Ticket requestors were not added properly.",
            )
            # get history
            ...
            self.assertEqual(
                hist[0]["Content"],
                ticket_text,
                "Ticket text was not receives is it was submited.",
            )
            # get_short_history
            ...
            # create 2nd ticket
            ticket2_subject = "Testing issue " + "".join(
                [random.choice(string.ascii_letters) for i in range(15)]
            )
            ...
            self.assertTrue(
                "DependsOn" in links1, "Missing just created link DependsOn."
            )
            self.assertTrue(
                links1["DependsOn"][0].endswith("ticket/" + str(ticket2_id)),
                "Unexpected value of link DependsOn.",
            )
            links2 = tracker.get_links(ticket2_id)
            self.assertTrue(
                "DependedOnBy" in links2, "Missing just created link DependedOnBy."
            )
            self.assertTrue(
                links2["DependedOnBy"][0].endswith("ticket/" + str(ticket_id)),
            ...
            self.assertTrue(
                at_list, "Empty list with attachment ids, something went wrong."
            )
            at_names = [at[1] for at in at_list]
            self.assertTrue(
                attachment_name in at_names,
                "Attachment name is not in the list of attachments.",
            )
            ...
            self.assertEqual(
                at_content,
                attachment_content,
                "Recorded attachment is not equal to the original file.",
            )
            # merge tickets
            self.assertTrue(
                tracker.merge_ticket(ticket2_id, ticket_id), "Merging tickets failed."
            )
            # delete ticket
            self.assertTrue(
                tracker.edit_ticket(ticket_id, Status="deleted"),
    ...
    def test_ticket_operations_admincc_cc(self):
        ...
        for name in ("RT4.4 stable",):
            ...
            admincc = []
            tracker.edit_ticket(
                ticket_id, Status="open", Requestors=requestors, AdminCc=admincc
            )
            # verify
            ticket = tracker.get_ticket(ticket_id)
            self.assertListEqual(requestors, ticket["Requestors"])
