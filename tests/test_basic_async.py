...
async def test_invalid_api_url():
    with pytest.raises(ValueError):
        rt.rest2.AsyncRt(
            url="https://example.com", http_auth=httpx.BasicAuth("dummy", "dummy")
        )
...
async def test_ticket_operations(async_rt_connection: rt.rest2.AsyncRt):
    ...
    # empty search result
    search_result = [
        item async for item in async_rt_connection.search(Subject=ticket_subject)
    ]
    ...
    search_result = [
        item
        async for item in async_rt_connection.search(
            Subject=ticket_subject, query_format=["Status", "Subject"]
        )
    ]
    assert len(search_result) == 1
    ...
    search_result = [
        item
        async for item in async_rt_connection.search(
            raw_query=f'Subject="{ticket_subject}"'
        )
    ]
    assert len(search_result) == 1
    assert search_result[0]["id"] == str(ticket_id)
    ...
    await async_rt_connection.edit_ticket(
        ticket_id, Status="open", Requestor=requestors
    )
    ...
    for hyperlink in transaction["_hyperlinks"]:
        if hyperlink["ref"] == "attachment":
            ...
            attachment = base64.b64decode(
                (await async_rt_connection.get_attachment(attachment_id))["Content"]
            ).decode("utf-8")
            found = True
    ...
    # get_short_history
    short_hist = [
        item async for item in async_rt_connection.get_ticket_history(ticket_id)
    ]
    ...
    ticket2_id = await async_rt_connection.create_ticket(
        queue=RT_QUEUE, subject=ticket2_subject
    )
    assert ticket2_id > -1
    ...
    assert await async_rt_connection.reply(
        ticket_id, content=reply_text, attachments=[attachment]
    )
    ...
    assert await async_rt_connection.reply(
        ticket_id, content=reply_text, content_type="text/html"
    )
    ...
    at_content = base64.b64decode(
        (await async_rt_connection.get_attachment(at_id))["Content"]
    )
...
async def test_attachments_create(async_rt_connection: rt.rest2.AsyncRt):
    ...
    for _i in range(attachment_count):
        ...
        attachments.append(
            rt.rest2.Attachment(attachment_name, "text/plain", attachment_content)
        )
...
async def test_attachments_comment(async_rt_connection: rt.rest2.AsyncRt):
    ...
    ticket_text = "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
    comment_success = await async_rt_connection.comment(
        ticket_id=ticket_id, content=ticket_text, attachments=attachments
    )
...
async def test_attachments_reply(async_rt_connection: rt.rest2.AsyncRt):
    ...
    # comment with attachments
    ticket_text = "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
    comment_success = await async_rt_connection.reply(
        ticket_id=ticket_id, content=ticket_text, attachments=attachments
    )
    assert comment_success
...
async def test_ticket_operations_admincc_cc(async_rt_connection: rt.rest2.AsyncRt):
    ...
    admincc = []
    await async_rt_connection.edit_ticket(
        ticket_id, Status="open", Requestor=requestors, AdminCc=admincc
    )
    # verify
    ticket = await async_rt_connection.get_ticket(ticket_id)
    assert compare_list(requestors, ticket["Requestor"])
...
async def test_users(async_rt_connection: rt.rest2.AsyncRt):
    ...
    assert (
        await async_rt_connection.user_exists("tester1@example.com", privileged=False)
        is True
    )
    assert (
        await async_rt_connection.user_exists("tester1@example.com", privileged=True)
        is False
    )
    ...
    assert (
        await async_rt_connection.create_user(
            random_user_name, random_user_email, Password=random_user_password
        )
        == random_user_name
    )
    ...
    assert await async_rt_connection.edit_user(
        user_id=random_user_name, RealName=random_user_name
    )
...
async def test_queues(async_rt_connection: rt.rest2.AsyncRt):
    ...
    await async_rt_connection.create_queue(
        random_queue_name,
        CorrespondAddress=random_queue_email,
